...
process prefetch {
  storeDir "${params.sraoutdir}" //storing moves the output of the process to store directory from the work folder
...
process fastqdump {
  container "https://depot.galaxyproject.org/singularity/sra-tools:2.11.0--pl5262h314213e_0"
  publishDir "${params.sraoutdir}", mode: "copy", overwrite: true // copies output to the mentioned dir.
  input:
...
process fastp {
  ...
  publishDir "${params.sraoutdir}/fastp", mode: "copy", overwrite: true
  input:
    path infastq
    val accession
  output:
...
process fastqc {
  publishDir "${params.sraoutdir}", mode: "copy", overwrite: true
...
workflow sradownload {
  take:
    accession
  main:
    params.sraoutdir = params.outdir + "/data"
    ...
    fastpout = fastp(fastqs, accession)
    // this channel has both trimmed and original fastqs!
    all_fastqs = fastqs.concat(fastpout.fastq_cut)
    qcied = fastqc(all_fastqs.flatten(), accession)
    multiqc_input_channel = fastpout.fastpreport.concat(qcied.zipped)


  emit:
    fastq_trimmed = fastpout.fastq_cut
    multiqc_input = multiqc_input_channel.collect()

}


workflow {
  sraout = sradownload(params.accession)
  sraout.fastq_trimmed.view()
  multiqc(sraout.multiqc_input, params.accession)
}
